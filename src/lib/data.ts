...
// Add this function to your lib/data.ts file

export const getCurrentAccountant = async (clerkUserId: string) => {
  try {
    const accountant = await prisma.accountant.findFirst({
      where: {
        username: clerkUserId, // Assuming username stores the Clerk user ID
      },
    });
    return accountant;
  } catch (error) {
    console.error("Error fetching current accountant:", error);
    return null;
  }
};

// Also add these functions for the UserCard component
export const getTotalFees = async () => {
  try {
    return await prisma.fee.count();
  } catch (error) {
